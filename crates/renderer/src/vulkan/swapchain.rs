...
use ash::{Device, Instance, khr::swapchain as khr_swapchain, vk};

use super::{VulkanDevice, VulkanSurface, VulkanSurfaceError};
...
impl Display for VulkanSwapchainError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Vulkan(error) => {
                write!(formatter, "Vulkan swapchain error: {error:?}",)
            }
            ...
            Self::NoSurfaceFormats => {
                write!(formatter, "surface exposes no usable formats",)
            }

            Self::NoPresentModes => {
                write!(formatter, "surface exposes no presentation modes",)
            }
...
impl VulkanSwapchain {
    ...
    fn create(
        ...
        if formats.is_empty() {
            return Err(VulkanSwapchainError::NoSurfaceFormats);
        }

        if present_modes.is_empty() {
            return Err(VulkanSwapchainError::NoPresentModes);
        }
        ...
        let composite_alpha = choose_composite_alpha(capabilities.supported_composite_alpha)
            .ok_or(VulkanSwapchainError::NoCompositeAlphaMode)?;

        let required_usage =
            vk::ImageUsageFlags::COLOR_ATTACHMENT | vk::ImageUsageFlags::TRANSFER_DST;

        if !capabilities.supported_usage_flags.contains(required_usage) {
            return Err(VulkanSwapchainError::UnsupportedImageUsage);
        }
        ...
        let loader = khr_swapchain::Device::new(instance, device.raw());
...
fn create_image_views(
    ...
    images: &[vk::Image],
    format: vk::Format,
) -> Result<Vec<vk::ImageView>, VulkanSwapchainError> {
