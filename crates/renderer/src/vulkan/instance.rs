...
pub enum VulkanInitError {
    ...
    UnsupportedApiVersion { required: u32, available: u32 },
...
impl VulkanInstance {
    ...
    pub fn for_display(display_handle: RawDisplayHandle) -> Result<Self, VulkanInitError> {
        let extensions = ash_window::enumerate_required_extensions(display_handle)?;

        Self::with_extensions(extensions)
    }

    fn with_extensions(required_extensions: &[*const c_char]) -> Result<Self, VulkanInitError> {
        ...
        if loader_version < TARGET_API_VERSION {
            return Err(VulkanInitError::UnsupportedApiVersion {
        ...
        if VALIDATION_ENABLED {
            instance_create_info = instance_create_info.enabled_layer_names(&validation_layers);
    ...
    pub fn physical_devices(&self) -> Result<Vec<PhysicalDeviceInfo>, VulkanInitError> {
...
fn validation_layer_available(entry: &Entry) -> Result<bool, VulkanInitError> {
...
fn create_debug_messenger_info() -> vk::DebugUtilsMessengerCreateInfoEXT<'static> {
